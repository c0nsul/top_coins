...
const Liquidity = () => {
    ...
    const updateDetails = (index) => {
        ...
        response.data.forEach((coin2, index2) => {
            if (index2 === index) {
                setDetails(`Name: ${coin2.name}, 
                    Market Cap: ${new Intl.NumberFormat("en-US", {
                    style: "currency",
                    currency: "USD"
                }).format(coin2.quote.USD.market_cap)}, 
                volume: ${new Intl.NumberFormat("en-US", {
                    style: "currency",
                    currency: "USD"
                }).format(coin2.quote.USD.volume_24h)}, 
                price change: ${coin2.quote.USD.percent_change_1h.toFixed(2)}%`)
            }
        })
    }
